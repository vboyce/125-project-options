...
raw_raw <- read_csv(here(raw_data_loc, "raw_pilot_B.csv")) %>% 
  ...
  #filter(StartDate > as.Date("2024-02-01")) %>% 
  write_csv(here(raw_data_loc,"anon_pilot_B.csv"))
```

```{r data cleaning, include=T}
raw_dat <- read_csv(here(raw_data_loc, "anon_pilot_B.csv"), col_types=cols(.default="c"))
...
data %>% write_csv(here(raw_data_loc, "pilot_data.csv"))
```
